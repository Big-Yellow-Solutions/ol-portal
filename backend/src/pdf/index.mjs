...
/* `anchor` marks the blank block DocuSign should autoplace a signature/date
   tab onto — invisible (white-on-white, 1px) so it never appears in a normal
   downloaded PDF, but still present in the text layer DocuSign's AutoPlace
   scans. Only the client side ever needs one: OL's signature is always
   captured natively, DocuSign or not (see docusign.mjs). */
function signatureBlock(sig, fallbackLabel, anchor) {
  const anchorTag = anchor
    ? `<span style="font-size:1px;color:#fff">[[ds_sig_${anchor}]] [[ds_date_${anchor}]]</span>`
    : "";
  if (!sig) return `<div class="line">${anchorTag}${esc(fallbackLabel)} &mdash; date</div>`;
...
function certificatePage(d) {
  ...
  const row = (label, sig) => `
    <table class="info cert">
      ...
      <tr><td class="k">Method</td><td>${esc(
        sig.signatureType === "drawn" ? "Drawn signature"
        : sig.signatureType === "docusign" ? "Signed via DocuSign"
        : "Typed signature"
      )}</td></tr>
      ...
      ${sig.signatureType === "docusign" ? `<tr><td class="k">Evidence</td><td>Full signing evidence (IP, device, consent language, and timestamps) is recorded in DocuSign's own Certificate of Completion for this envelope.</td></tr>` : ""}
    </table>`;
    ...
    * { box-sizing: border-box; margin: 0; }
    ...
    <div class="sign">
      ...
      <div class="col">
        ...
        ${signatureBlock(d.signatures?.client, d.signatureRight, "client")}
      </div>
    </div>
